...
- **Exercise Type** — the interaction primitive of an Exercise. Exactly two: **Choice** (pick one of N options) and **Line-Select** (tap the offending line).
- **Flavor** — an authoring tag on a Choice Exercise describing what is being asked (e.g. predict-output, predict-error, pick-refactor). Drives labels and stats, not engine behaviour.
