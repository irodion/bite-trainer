...
- **Pack** — a distributable content unit for one programming language, loadable from any URL. Contains Topics. Identified by the id it declares, never by where it was loaded from.
- **Pack Source** — the URL a Pack is currently loaded from. Can change without the Pack becoming a different Pack.
...
- **Progress Log** — the append-only event log of a Learner's attempts, completed Sessions and progress resets; the sole source of progress state.
- **Instance** — one browser storage area holding its own Progress Log (e.g. a Safari tab and the installed app are two). One Learner may have several; export/import moves progress between them.
