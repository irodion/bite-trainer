...
const attempt = (at: number, correct: boolean, elapsedMs = 1000): LogEvent => ({
  type: 'attempt', id: `e${n++}`, instanceId: 'i', at, tzOffsetMin: 0, sessionId: 's',
  packId: 'p', packVersion: '1', exerciseId: 'x', answer: { optionId: 'a' }, correct, elapsedMs, timeBudget: 60,
})
...
test('reset hides earlier attempts', () => {
  const ev: LogEvent[] = [attempt(0, true), { type: 'reset', id: 'r', instanceId: 'i', at: 5, tzOffsetMin: 0, packId: 'p' }]
  expect(schedule(ev, 'p').size).toBe(0)
})

test('streak counts consecutive local days, tolerating today not done yet', () => {
  const done = (at: number): LogEvent => ({ type: 'session-completed', id: `d${at}`, instanceId: 'i', at, tzOffsetMin: 0, sessionId: 's' })
