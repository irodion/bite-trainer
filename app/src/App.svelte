...
{:else}
  <main>
    ...
    <h2>Topics</h2>
    <ul>
      {#each app.pack.topics as t (t.id)}
    ...
    <div class="row">
      ...
      <label class="btn-file"
        >Import<input type="file" accept="application/json,.json" onchange={importLog} hidden /></label
      >
      <button onclick={() => record({ type: 'reset', packId: app.pack!.manifest.id })}
        >Erase progress for this Pack</button
      >
    </div>
