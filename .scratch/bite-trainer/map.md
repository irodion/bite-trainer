...
- [What Topics does the official Rust Pack contain, and how is it authored?](issues/09-rust-curriculum-outline.md) — 48 Exercises in 10 Topics adapted directly from rustlings (broken version → predict-error / find-the-line, solution → predict-output / pick-refactor), each with `source` attribution; Rationales still need a human read.
- [What author-time verification must a Pack pass?](issues/10-pack-verification.md) — `pnpm verify-pack`: structure + 30×60 limits + a mandatory machine-checkable Claim per Exercise, proven by `rustc` behind a generic `Toolchain` interface; runs in the pre-commit gate.
...
- 2026-09-19 — User's first real Session exposed it at once: every correct option was authored as `a` and the app did not shuffle, so position gave the answer away. Fixed in the app, not the content: `core/shuffle.ts` (tested for fairness), options presented in a fresh order each time, letters follow display position, the Progress Log keeps recording the option id. `verify-pack` now rejects options that refer to each other by position ("none of the above", "option B"). Lesson: a finding from research ("shuffle option order") that is not turned into a test gets lost.
- 2026-09-19 — Leave / continue / end-of-Session built (see the Session and Mastery ticket's comments). An unfinished Session is derived from the log, not stored. Screenshot review caught a floating ✕ covering code on line 1 → moved into the sheet header. "Done for today" + "Practice more" replaced an endless Start button.
